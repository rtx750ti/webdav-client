...
pub trait UrlFormat {
    ...
    /// ## 示例
    /// ```
    /// # use webdav_client::_client::error::WebDavClientError;
    /// # use webdav_client::_client::traits::url_trait::UrlParse;
    /// # use webdav_client::_client::{WebDavClient, structs::webdav_child_client::WebDavChildClientKey};
    /// # async fn example() -> Result<(), WebDavClientError> {
    /// let _client = WebDavClient::new();
    /// let key = WebDavChildClientKey::new(
    ///     "https://dav.example.com/dav/我的坚果云/",
    ///     "username"
    /// )?;
    ///
    /// let url_str = _client.format_url_path(&key, "./书签").await?;
    /// assert_eq!(
    ///     url_str,
